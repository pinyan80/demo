...
public class HibernateUtil_CompositeQuery_morningcode {
	...
	public static Predicate get_aPredicate_For_AnyDB(CriteriaBuilder builder, Root<MealsVO> root, String columnName, String value) {
		...
		if ("mealsId".equals(columnName)) // 用於Integer
			predicate = builder.equal(root.get(columnName), Integer.valueOf(value));
		else if ("mealsPrice".equals(columnName) || "mealsPrice".equals(columnName)) // 用於Double
			predicate = builder.equal(root.get(columnName), Double.valueOf(value));
		else if ("mealsName".equals(columnName) || "mealsName".equals(columnName)) // 用於varchar
			predicate = builder.like(root.get(columnName), "%" + value + "%");
		else if ("mealsTypesId".equals(columnName)) {
			MealsTypesVO mealsTypesVO = new MealsTypesVO();
			mealsTypesVO.setMealsTypesId(Integer.valueOf(value));
			predicate = builder.equal(root.get("mealsTypesVO"), mealsTypesVO);
		}
...
//		Session session = HibernateUtil.getSessionFactory().openSession();
		...
		try {
			...
			criteriaQuery.orderBy(builder.asc(root.get("mealsId")));
